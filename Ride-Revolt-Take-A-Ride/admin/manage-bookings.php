...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_REQUEST['eid'])) {
        ...
        // Set the vehicle's isBooked status to 'false' when the booking is cancelled
        $sqlUpdateVehicle = "UPDATE tblvehicles SET isBooked = 0 WHERE id = (SELECT VehicleId FROM tblbooking WHERE id = :eid)";
        $queryUpdateVehicle = $dbh->prepare($sqlUpdateVehicle);
        $queryUpdateVehicle->bindParam(':eid', $eid, PDO::PARAM_STR);
        $queryUpdateVehicle->execute();
    ...
    if (isset($_REQUEST['aeid'])) {
        ...
        // Set the vehicle's isBooked status to 'true' when the booking is confirmed
        $sqlUpdateVehicle = "UPDATE tblvehicles SET isBooked = 1 WHERE id = (SELECT VehicleId FROM tblbooking WHERE id = :aeid)";
        $queryUpdateVehicle = $dbh->prepare($sqlUpdateVehicle);
        $queryUpdateVehicle->bindParam(':aeid', $aeid, PDO::PARAM_STR);
        $queryUpdateVehicle->execute();
...
<head>
    ...
    <title>Ride Revolt-Make A Ride | Admin Manage bookings </title>
    <!-- Font awesome -->
